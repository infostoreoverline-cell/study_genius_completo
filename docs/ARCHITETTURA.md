...
La finestra di lavoro dei capitoli è distinta dai limiti API: con i valori predefiniti possono avanzare fino a quattro capitoli tra scrittura, compilazione e review, ma i semafori continuano ad ammettere al massimo due richieste Gemini e due DeepSeek. Dopo la lettura, indice DeepSeek e guida Gemini avanzano contemporaneamente; se uno fallisce, il fratello viene cancellato prima di chiudere i client. Le anteprime dei capitoli compilano lo stesso corpo e gli stessi asset in un solo passaggio, mentre il libro finale conserva fino a quattro passaggi per stabilizzare indice e riferimenti. La compatibilità degli endpoint strutturati viene ricordata localmente per versione e modello; il fallback conserva la validazione completa e non viene riprovato inutilmente a ogni nuovo progetto.
